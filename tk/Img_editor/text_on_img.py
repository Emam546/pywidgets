...
class Text_putter:
    def __init__(self,root:Tk,*images, box, target, radius_selcting=10,
        ...
        self.drawed_images=list(images)
        self.box=box
    ...
    def _update_images(self):
        booled_img=self.maskimg.astype(np.bool)
        b=self.box
        for i,img in enumerate(self.original_images):
            drawed_img=img.copy()
            color=255 if len(img.shape)==2 else self.text_color
            drawed_img[b[1]:b[1]+b[3],b[0]:b[0]+b[2]][booled_img]=color
            self.drawed_images[i]=drawed_img
        return self.drawed_images
    def motion(self, pos):
        index=self.selecting_box._get_grabing_state(pos)
        if index!=None:
        ...
        self.target(*self.drawed_images)
...
class _Text_getter(Text):
    ...
    def mouse_pos(self):
        ...
        if len(lines)>0:
            ...
            #this algourism is for ensure that we will get the right index
            #with the arrangement
            rows,columns=str(self.index(INSERT)).split(".")
            rows=int(rows)-1
            columns=max(0,int(columns))
            org_total_letters=0
            #if the cursor in the finhied
            finihed=False
    ...
    def cursor_cor(self,pos):
        ...
        for r,line in enumerate(lines):
            ...
            for letter in line:
                ...
                if not count_all:
                    cuuretn_width+=self._drawer.textsize(letter,self.font, self.spacing, self.direction, self.features, self._language, self.stroke_width)[0]
                    #getting the best 
                    char_distance=distance([cuuretn_width,height],pos)
                    if max_x_distance<=char_distance:
                        #check if it is the most perfect row or not by comaparse with next row height
                        #getting frist row heigth
                        height2=line_spacing*(r+2)
                        current_distance=distance([0,height],pos)
                        next_ditance=distance([0,height2],pos)
                        #if it is lower distance than next row break the insied llob
                        if next_ditance>current_distance:
                            #it is the same distance
                            #finih the loob
                            #break  insede the loob
                            break
                        else:
                    ...
                    total_org_characters+=1
            #check if the loob ended by break or not
            else:
                #if not continue searching
                continue
            print("break")
            #if True break the llob and send the totla number of letters
            break
